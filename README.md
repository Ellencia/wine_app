...
`raw_bottles/` — 카탈로그 PDF에서 추출한 원본 해상도 보틀 이미지 130개 (git 제외, 로컬 전용).
114개는 PDF에 임베디드된 300dpi 개별 이미지를 알파 마스크째 추출한 **투명 누끼**이고,
16개는 배경 스캔에 병이 박혀 있던 페이지라 여백을 넉넉히 준 300dpi 크롭(배경 포함)임.
앱용 `public/images/wines/` 이미지를 다시 만들 때 이 폴더를 원본으로 쓸 것.
재추출 스크립트는 세션 스크래치패드의 extract_bottles.py (검출 박스 + 임베디드 이미지 겹침 매칭).
